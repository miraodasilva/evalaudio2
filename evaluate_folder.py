...
parser.add_argument("--resample_24khz", action="store_true", help="If true, resample fake audio from 24 to 16 khz")
...
for root, _, files in os.walk(args.fake_folder):
    for f in tqdm(files, desc=root):
        ...
        elif args.resample_24khz:
            resample = torchaudio.transforms.Resample(24_000, 16000)
            audio_fake = resample(audio_fake)
        ...
        annotation = (
            audio_fake_path.replace(args.fake_folder, annotation_folder).replace(".wav", ".align")
        ...
        wer += [
            wer_model(
                audio_fake.view(1, 1, -1), [audio_fake.size(-1)], annotations=[annotation], names=[audio_fake_path]
            )
        ]
